...
def get_or_create_worksheet(sh, name):
    try: 
        ...
        return sh.add_worksheet(title=name, rows="1000", cols="50")
...
def charger_base_cloud():
    try:
        ...
        st.sidebar.error("❌ Fichier Google Sheets 'Base_Calanques_DB' introuvable.")
...
def sauvegarder_base_cloud(db):
    try:
        ...
        ws.clear()
        ws.update(values=chunks, range_name="A1")
    except Exception as e:
...
if 'db' not in st.session_state: 
    with st.spinner("Connexion sécurisée au Cloud Google..."):
        ...
        for cle in ["validations_promo", "sorties_manuelles", "cartes_membres", "eleves_deja_affectes", "identites_helloasso_connues"]:
            if cle not in st.session_state['db']: st.session_state['db'][cle] = [] if type(initialiser_memoire_vierge()[cle]) is list else {}
...
def analyser_fichier_ffe(fichier):
    try:
        if isinstance(fichier, str):
            if fichier.endswith('.csv'): df_ffe = pd.read_csv(fichier, sep=None, engine='python')
                ...
                if all_data:
                    ...
                    if not nouveaux.empty:
                        ...
                        for _, row in nouveaux.iterrows():
                            ...
                            if identite not in st.session_state['db']['identites_helloasso_connues']:
                                st.session_state['db']['identites_helloasso_connues'].append(identite)
